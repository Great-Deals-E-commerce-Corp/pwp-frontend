...
export default function CampaignModulesPage() {
  ...
  const defaultTab = searchParams.get('tab') || CAMPAIGN_TYPES[0];
  const [activeTab, setActiveTab] = useState<CampaignType>(defaultTab as CampaignType);
  ...
  const campaignsForRole = useMemo(() => {
    ...
    if (userRole === 'commercial') {
      return campaigns.filter(c => c.createdBy === 'commercial@demo.com');
    }
  ...
  const uniqueBrands = useMemo(() => {
    const brands = new Set(campaignsForRole.map(c => c.brandName).filter(Boolean) as string[]);
    return Array.from(brands).sort();
  }, [campaignsForRole]);

  const filteredData = useMemo(() => {
    return campaignsForRole.filter(campaign => {
      if (campaign.campaignType !== activeTab) return false;
      ...
      return matchesSearch && matchesStatus && matchesBrand && afterFilterStart && beforeFilterEnd;
    });
  }, [campaignsForRole, searchTerm, statusFilter, brandFilter, startDateFilter, endDateFilter, activeTab]);
  ...
  const handleExport = () => {
    ...
    link.setAttribute('download', `campaign_modules_${activeTab.replace(' ','_').toLowerCase()}_export.csv`);
  ...
  return (
    <div className="flex flex-col gap-8">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold">Campaign Type</h1>
          <p className="text-muted-foreground">
            View and manage all campaign types.
          </p>
        </div>
      ...
      <Tabs value={activeTab} onValueChange={(value) => setActiveTab(value as CampaignType)} className="w-full">
        ...
        {CAMPAIGN_TYPES.map(type => (
            <TabsContent key={type} value={type} className="mt-4">
                <CampaignTable
                    ...
                    description={`A list of all ${type} campaigns in the system.`}
                    userRole={userRole}
